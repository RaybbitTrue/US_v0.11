...
class Container4(Screen):
	...
	def Check_Display_Notes(self):
		if self.display_notes.text != "":		# если поле ввода заметки пусто, то:
			self.Add_notes()					# то запускаем функцию Add_notes
		else:
			pass

	# функция проверки наличия текста в поле ввода заметки:
	def Check_Display_Notes_For_Update_Note(self):
		if self.display_notes.text != "":		# если поле ввода заметки пусто, то:
			self.Update_note()					# то запускаем функцию Add_notes
		else:
			pass
	...
	def Add_notes(self):
		...
		#Container5.Add_New_Button(Container5())
